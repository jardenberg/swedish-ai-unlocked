...
import {
  ...
  scrapeBatchCore,
  embedBatchCore,
} from "./ingest-core.server";

export { BulkGuardError };
...
export const embedBatch = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertAdmin(context.supabase, context.userId);
    return await embedBatchCore({ ...data, trigger: "manual" });
  });
...
export const refreshSitemap = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertAdmin(context.supabase, context.userId);
    return await refreshSitemapCore({ ...data, trigger: "manual" });
  });
...
export const publicStats = createServerFn({ method: "GET" }).handler(async () => {
  ...
  return {
    ...
    chunks: chunkCount ?? 0,
    sources: sources ?? [],
    lastRefresh: (lastRun as { finished_at?: string | null } | null)?.finished_at ?? null,
  };
});
