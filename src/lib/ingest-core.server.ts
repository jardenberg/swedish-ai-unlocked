...
export const EMBEDDED_DROP_GUARD = 0.1;

const FALLBACK_PACE_MS = 6_500;
...
function isRateLimitError(error: unknown): boolean {
  ...
  return candidate.status === 429 || /rate limit/i.test(candidate.message ?? "");
}
...
// ──────────────────────────────────────────────────────────────────
export async function refreshSitemapCore(args: {
  sourceSlug: "rise" | "ai_sweden";
  force?: boolean;
  trigger?: Trigger;
}) {
  const data = args;
  ...
  for (const doc of existingRows) {
    // Parked rows are intentionally excluded from automatic refreshes.
    if (doc.status === "skipped_offsite" || doc.status === "parked_unscrapable") continue;
    const newLastmod = lastmodByUrl.get(canonicalizeUrl(doc.url));
    if (
      newLastmod &&
      (!doc.sitemap_lastmod || new Date(newLastmod) > new Date(doc.sitemap_lastmod))
    ) {
...
// ──────────────────────────────────────────────────────────────────
export async function scrapeBatchCore(args: {
  sourceSlug: "rise" | "ai_sweden";
  batchSize: number;
  trigger?: Trigger;
}) {
  ...
  const sourceHost = (() => {
    try {
      return new URL(source.root_url).hostname.replace(/^www\./, "");
    } catch {
      return "";
    }
  })();
  ...
  const markSkippedOffsite = async (
    docId: string,
  ...
  if (htmlDocs.length > 0) {
    try {
      ...
      const batchRes = await fc.batchScrape(urls, { options: scrapeOpts } as unknown as Parameters<
        typeof fc.batchScrape
      >[1]);
      const docs =
        (
          batchRes as {
      ...
      credits += docs.length;
      // Firecrawl reports the requested URL in sourceURL and the resolved
      // destination in url. Key by the request so same-site redirects still
      // attach to their original document row.
      const byUrl = new Map<
        string,
        { markdown: string; title?: string; rawHtml?: string; finalUrl?: string }
      >();
      for (const d of docs) {
        const key = d.metadata?.sourceURL ?? d.metadata?.url;
        const final = d.metadata?.url ?? d.metadata?.sourceURL;
        if (key && d.markdown)
          byUrl.set(key, {
            markdown: d.markdown,
      ...
      for (const doc of htmlDocs) {
        ...
        if (!got || got.markdown.length <= 100) {
          try {
            const elapsed = Date.now() - lastFallbackStartedAt;
            if (lastFallbackStartedAt > 0 && elapsed < FALLBACK_PACE_MS) {
              await sleep(FALLBACK_PACE_MS - elapsed);
            }
            lastFallbackStartedAt = Date.now();
            type RetryDocument = {
              markdown?: string;
              rawHtml?: string;
              metadata?: { sourceURL?: string; url?: string; title?: string };
            };
            let retry: RetryDocument | null = null;
            for (let attempt = 0; attempt < 2; attempt++) {
              try {
                retry = (await fc.scrape(doc.url, {
                  formats: ["markdown", "rawHtml"],
                  onlyMainContent: true,
                  waitFor: 1500,
                  excludeTags,
                } as unknown as Parameters<typeof fc.scrape>[1])) as unknown as RetryDocument;
                break;
              } catch (error) {
            ...
            credits += 1;
            const retryFinal = retry?.metadata?.url ?? retry?.metadata?.sourceURL;
        ...
        if (got && got.markdown.length > 100) {
          ...
          await supabaseAdmin
            .from("documents")
            .update({
              status: "parked_unscrapable",
              error:
                "parked: HTTP page returned no usable markdown after source selectors and paced onlyMainContent fallback",
              filter_miss: true,
            })
            .eq("id", doc.id);
        }
      ...
      if (res.text && res.text.length > 200) {
        ...
        await supabaseAdmin
          ...
          .update({
            ...
            published_at_source:
              res.publishedAt?.source ?? (doc.published_at ? doc.published_at_source : null),
  ...
  await supabaseAdmin
    ...
    .update({
      ...
      notes: skippedOffsite ? `skipped_offsite=${skippedOffsite}` : null,
    })
    .eq("id", run!.id);
...
// ──────────────────────────────────────────────────────────────────
export async function embedBatchCore(args: {
  sourceSlug?: "rise" | "ai_sweden";
  batchSize: number;
  trigger?: Trigger;
}) {
