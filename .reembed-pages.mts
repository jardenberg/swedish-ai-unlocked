...
await sb.from("documents").update({ hidden: false, status: "scraped" }).in("url", urls);
const { data: docs } = await sb.from("documents").select("*").in("url", urls);
for (const doc of docs ?? []) {
  ...
  await sb.rpc("replace_chunks", { p_document_id: doc.id, p_rows: rows });
  await sb.from("documents").update({ status: "embedded", error: null, filter_miss: false }).eq("id", doc.id);
  console.log("re-embedded", doc.url, "chunks:", chunks.length);
}
console.log("done");
